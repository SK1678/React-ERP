    ...
    ApiManager.testConnection()
      ...
      .catch(err => {
        this.setState({
          ...
          backendMessage: 'Failed to connect to Laravel'
        });
      });
  }
            ...
            <div className="card">
              <div className="card-body py-2 px-3">
                <div className="d-flex align-items-center justify-content-between">
                  <p className={`mb-0 ${this.state.backendStatus === 'Connected' ? 'text-success' : 'text-danger'}`}>
                    <strong>Database Status:</strong> {this.state.backendStatus}
                    {this.state.backendMessage && ` - ${this.state.backendMessage}`}
                  </p>
                  {this.state.backendStatus === 'Disconnected' && (
                    <small className="text-danger">Make sure Laravel is running on port 8001</small>
                  )}
                </div>
              </div>
            </div>
